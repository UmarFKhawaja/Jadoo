...
import { Ingredient, Photo, Solution, Step, Task, User } from '../../entities';
import { IngredientService, SolutionService, StepService, TaskService, UserService } from '../../services';
import { Context } from '../../types';
import { IngredientArgs, SolutionArgs, StepArgs, TaskArgs, UserArgs } from './types';
...
export class PhotoResolver {
  ...
  async solution(@Root() photo: Photo, @Args() args: SolutionArgs, @Ctx() context: Context): Promise<Solution | null> {
    const { id: photoID }: Photo = photo;

    const solution: Solution | null = await this.solutionService.findSolutionByPhotoID(photoID);
